...
window.addEventListener('load', () => {
  ...
  let openInstructionsButtons = document.querySelectorAll('[data-instructions-target]');
  let closeInstructionsButtons = document.querySelectorAll('[data-close-button]');
  let overlay = document.getElementById('overlay');

  openInstructionsButtons.forEach(button => {
    button.addEventListener('click', () => {
      let instructions = document.querySelector(button.dataset.instructionsTarget);
  ...
  // let jsonData = JSON.stringify(plantObj);
  ...
  //1. make a fetch request of type POST so that we can send the (plantdata) info to the server
...
function draw() {
  ...
  for (let i = 0; i < plants.length; i++) {
    ...
    // image(plants[i].img, plants[i].x, plants[i].y, 30 * unit, 30 * unit);
    showName(plants[i].x, plants[i].y, plants[i].username); // plant.name
    //to show every plant from plants array;
    if (tool) {
      //to see which tool is selected and grow or remove plant here
      if (mouseIsPressed) {
        if (dist(mouseX, mouseY, plants[i].x, plants[i].y) < 30 * unit) {
          // plants[i].imgIndex = flowers[plants[i].t][floor(map(plants[i].g, 0, 150, 0, 3))];
          plants[i].imgIndex = 1;
          plants[i].g++;
          if (plants[i].g > 140) {
            plants[i].g = 140;
  ...
  if (tool) {
    image(tool, mouseX - 15 * unit, mouseY - 15 * unit, 30 * unit, 30 * unit);
  }

  if (mouseIsPressed && plants.length > 0 && once == true) {
    //user clicks and hold mouse for planting/watering
...
function mouseReleased() {
  ...
  if (plants.length > 0) {
    if (
      ...
      // showName();     showName(plants[i].x, plants[i].y, plants[i].username); // plant.name
